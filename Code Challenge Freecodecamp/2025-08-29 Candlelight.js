...
function burnCandles(candles, leftoversNeeded) {
    let totalBurned = candles;  // Start by burning all initial candles
    let leftovers = candles;    // Initial leftovers from burning all candles
    
    while (leftovers >= leftoversNeeded) {
        let newCandles = Math.floor(leftovers / leftoversNeeded);
        leftovers = leftovers % leftoversNeeded;  // Remaining leftovers after making new candles
        
        totalBurned += newCandles;  // Add the new candles we're about to burn
        leftovers += newCandles;    // Add leftovers from burning the new candles
    }
